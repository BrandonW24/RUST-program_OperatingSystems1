...
fn main() {
    ...
    //Utilizing clone
    let data_part_1 = xs[0].clone(); 
    let data_part_2 = xs[1].clone();

    //As per the instructions we must call map_data here in order to create the two different threads
    //Having it be used on the two different partitions that were cloned.
    let thread_1 = thread::spawn( move || map_data(&data_part_1));
    let thread_2 = thread::spawn( move || map_data(&data_part_2));

    //Join threads together and then unrwp them
    let response_1 = thread_1.join().unwrap();
    let response_2 = thread_2.join().unwrap();

    //Add each of the responses together then push them.
    intermediate_sums.push(response_1);
    intermediate_sums.push(response_2);
    ...
    let partitions_info = partition_data(num_partitions, &v);

    // 2. Calls print_partition_info to print info on the partitions that have been created
    print_partition_info(&partitions_info);

    // 3. Creates one thread per partition and uses each thread to concurrently process one partition
    //Here the vector for keeping threads is made
    let mut threads : Vec<std::thread::JoinHandle<usize>> = Vec::new();
    let mut num_threads = 0;

    //Create as many threads as the user wants, spawning that number of threads
    while num_threads < num_partitions {
      let current_partition = partitions_info[num_threads].clone();
      threads.push(thread::spawn(move || {map_data(&current_partition)}));
      num_threads += 1;
    }

    //We create the vector to store the intermedite sum of all individual threads thread
    let mut intermediate_sums_data : Vec<usize> = Vec::new();

    // 4. Collects the intermediate sums from all the threads.
    // we utilize join here to collect all of the data and then we
    // push those intermidate sums.
    for num in threads {
        let end_result = num.join().unwrap();
        intermediate_sums_data.push(end_result);
    }

    // 5. Prints information about the intermediate sums
    println!("Intermediate sums = {:?}", intermediate_sums_data);
    // 6. Calls reduce_data to process the intermediate sums
    let sum = reduce_data(&intermediate_sums_data);
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    ...
    let mut p_counter = 0;
    let mut r_md = v.len();
    //Vector that holds vectors of data. vector-ception. it's like inception.
    let mut resulting: Vec<Vec<usize>> = Vec::new();
    let mut computed_data = 0;
    let mut index = computed_data;
    let mut remaining_d =  r_md % num_partitions;
    //For each partition present we want to create vectors corresponding to them.
    while p_counter < num_partitions {
         let mut partition : Vec<usize> = Vec::new();
         for _data_within_parts in 0..v.len() / num_partitions {
             if index != v.len() { 
                 if remaining_d != 0 {
                    while computed_data == 0 {
                        partition.push(v[index+1]);
                        remaining_d -= 1;
                        computed_data = 1;
                    }
                 }
                 partition.push(v[index]);
                 index += 1;
             }
         }
         //Reset our data tracker to zero for the next bit of data coming through.
         //We then push that data onto the resulting vector. At the end we return the result.
         computed_data = 0;
         resulting.push(partition);
         p_counter += 1;
    }
